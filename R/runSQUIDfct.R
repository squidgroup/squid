...
#' @param plot           logical; If \code{FALSE} (default), \code{runSQUIDfct} does not include the simulation plots into the output list (see Value).
...
#'
#' @return
#' \code{runSQUIDfct} returns a \code{list} that includes a \code{data.frame} of the full data generated, a \code{data.frame} 
...
#'
#' @details 
#' A detailed description of the SQuID model and its sampling design is provided on the SQuID app (run SQuID app with \code{\link{runSQUIDapp}}). Reading the SQuID documentation on the SQuID app is prerequisite to use properly the SQuID R function. 
#' 
...
#'  \tab \tab \tab \tab \cr
#'  \code{B} \tab \tab matrix(c(0,0,0,0), nrow=1) \tab \tab  1 row \code{matrix}; the population mean values. When the number of trait is set to 1, the dimension of the matrix is 1 row by 4 columns (1x4). 
#'                                                          The matrix elements are respectively the mean population at the intercept (b0),
...
#'                                                          the population mean response interaction to two environmental influences x1x2 (fixed slope; b12). If the number of trait is set to 2, the number of columns in the matrix 
#'                                                          is doubled (1x8) and the second half corresponds to the population mean values associated with the second trait.\cr
#'  \tab \tab \tab \tab \cr
