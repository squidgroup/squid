...
data <- sim_population(
  parameters = list(
    ...
    residual = list(
      names = c("environment","resid"),
      beta =c(0.5,1)
    )
  ),
  data_structure=make_structure("individual(300)",N=3000),
  model= "ind_int + environment + exp(ind_lnsd)*resid"
)
```
...
Indirect genetic effects are a bit more difficult to code 

 <!-- Finally we can also use indexing `[]` whereby we can alter the levels of the random effects within the formula. This is useful for indirect genetic effects. -->

This is where being able to index in the formula becomes important

Biology - Maternal genetic effects
...
NOTE: the maternal genetic effect returned for an individual, is that which it has the genes for, not that it has experienced
