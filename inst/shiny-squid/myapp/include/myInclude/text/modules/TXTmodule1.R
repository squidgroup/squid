...
Mod1Step1_txt <- list(
  ...
  "intro"      = paste("<b>Introduction:</b> This is the simplest possible situation. 
                        A trait measured in a group of individuals in a population is 
                        characterised by a mean and a variance ($V_",NOT$total,"$).",sep=""),
  ...
  "para1"      = paste("In every case we measure something, we know that we are making an error 
                  in the measurement. This error is assumed to be non-directional and hopefully 
                  to represent a small portion of the total variance $V_",NOT$total,"$. 
                  Try to play with the error term . 
  ...
  "point"      = paste("<b>Point:</b> The variance of measures ($V'_",NOT$total,"$) is often higher than the variance 
                  ...
                  Note that the estimated $V'_",NOT$total,"$ (total variance of sampled values) in not often equal to 1. 
  ...
  "solutions"  = "<b>Solutions:</b> First work to reduce the measurement error, and to do so, you need to know what 
                  it is. This requires measuring individuals more than once. 
...
Mod1Step2_txt <- list(    
  ...
  "intro"     = "<b>Introduction:</b> In the previous step, we have estimated the mean and the 
                  total variance of a sample of individuals measured once. You have 
                  learned that some of this variation might be due to measurement 
  ...
  "exercise"    = paste("<b>Exercise:</b> To simulate this new situation, we advise you to generate 
  ...
  "para1"   =  "You also need to enter how often each individual is measured. We call this 
                ...
                per individual affects how well your estimated values approach the true values.",
  ...
  "para3"   =  paste("The three histograms show the estimated total phenotypic variance $(V'_",NOT$total,")$, 
  ...
  "para4"   =  "A certain part of the variation is due to measurement error. 
                Measurement error can occlude true biological variance. We typically 
  ...
  "para6"   =  paste("The scatter plot shows how highly correlated the first and second 
...
Mod1Step3_txt <- list(  
  ...
  "intro"     = "<b>Introduction:</b> Individuals express different values of a trait at different times 
                        due to the influence of the environment. By collecting two or more measures 
                        for each individual it is thus possible to explore both how much variance is 
                        due to the environment and how the phenotype responds to the environment, 
                        but also to measure whether differences between individuals are solely 
                        due to differences in the environment at the time of measurement or to some 
                        other source, such as differences between individuals that were generated 
                        before the period of measurement.",
  "exercise"  = paste("<b>Exercise:</b> As for the two previous steps we are now generating a 
                        ...
                        and individual variance ($V_",NOT$devI,"$). This new variance represents specific 
                        and unknown environmental effects ($",general_VAR$EnvSpecUnk,"$) 
                        on the phenotype of an individual. A specific environmental effect means that each 
                        ...
                        An unknown environmental effect represents environment values that are not 
                        ...
                        to see how $",general_VAR$EnvSpecUnk,"$ changes 
  ...
  "point"    = paste("<b>Point:</b> We can now estimate a particular variance component of $V_",NOT$total,"$ that 
                        represents among-individual differences that are consistent through time. That variance is also 
                        in some circumstance the index for individual personality differences. The residual variance 
                        regroups both measurement error ($V_",NOT$error,"$) and the variance caused by the unmeasured (unknown) specific 
                        environment to each individual ($",general_VAR$EnvSpecUnk,"$). $",general_VAR$EnvSpecUnk,"$ 
                        ...
                        Repeatability is not simply a measure of your skill at measuring phenotypes. 
                        Instead, it now is an estimate of a biological phenomenon, consistent individual 
                        differences (personality for behaviour). The estimate is conservative because 
                        measurement error reduces the  estimate to be less than the true biological repeatability. 
                        Note that in this particular simulation, the model gives an estimate of repeatability 
                        that depends on $V_",NOT$residual,"$, the sum of both $",general_VAR$EnvSpecUnk,"$ and
  ...
  "para2"    = "You can now go to module 2 to understand different types of phenotypic measures, 
                such as events (yes or no), proportions, and counts."
)
...
Mod1Step4_txt <- list( 
  ...
  "intro"     = paste("<b>Introduction:</b> In step 3, we introduced $",general_VAR$EnvSpecUnk,"$ 
                as the variance caused by the environment. 
                We did not know what that effect was (it was unknown), 
 ...
 "step4_exercise"    = paste("<b>Exercise:</b> As before, we will generate a new group of individuals, 
                ...
                ($",general_VAR$EnvSpecKno,"$) on the phenotype of an individual.<br> 
                As before, you can set $V_",NOT$error,"$, $V_",NOT$devI,"$  and $",general_VAR$EnvSpecKno,"$, 
  ...
  "para1"    = "At this point, we want to introduce the idea of &lsquo;analysis&rsquo; models. 
                ...
                In the made-up world we are creating with our simulation, these models have 
                the potential of describing it exactly. The real world is different, 
                and most of the lessons we will cover have to do with problems in 
                inferring some terms in analysis models when much is unknown. 
  ...
  "para2"    = paste("where $",NOT$trait.1,"_{",NOT$time,NOT$ind,"}$ is the phenotype 
                ...
                $",NOT$devI,"_",NOT$ind,"$ is the individual mean phenotype, 
                $",NOT$mean,"_{",NOT$specific,NOT$known,"}$ is the effect of measured 
                environment $",NOT$env,"_{",NOT$specific,NOT$known,NOT$time,NOT$ind,"}$ on the measure of phenotype, 
                and $",NOT$error,"_{",NOT$time,NOT$ind,"}$ is the error made in that measurement.
                Note that $",general_VAR$EnvSpecKno,"=V(",NOT$mean,"_{",NOT$specific,NOT$known,"}",NOT$env,"_{",NOT$specific,NOT$known,"})=
                ",NOT$mean,"_{",NOT$specific,NOT$known,"}^2V(",NOT$env,"_{",NOT$specific,NOT$known,"})$
                where $V(",NOT$env,"_{",NOT$specific,NOT$known,"})=1$.",sep=""),
  "para3"    = "A mixed statistical model estimates these parameters:",
  "para4"    = paste("Visually, you can see what $",NOT$mean,"_{",NOT$specific,NOT$known,"}$ 
  ...
  "para6"    = paste("We encourage you to go back and play around with the magnitude of $V_",NOT$error,"$ 
                to see how it affects estimates of  $",NOT$mean,"_{",NOT$specific,NOT$known,NOT$time,"}$
  ...
  "point"    = "<b>Point:</b> This exercise introduced explanatory variables (also known as fixed effects). 
                ...
                strongly affects inference based on hypothesis testing. More importantly, 
