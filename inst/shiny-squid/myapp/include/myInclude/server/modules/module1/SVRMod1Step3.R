...
SVRMod1Step3 <- function(input, output, session, color){
  
  return(c(
  
    ######### Set variables #########    
      # Set hidden variables (Tmax, Vi, X1_state, X1_ran_V and NR)
       output$Mod1Step3_hidden <- renderUI({
          list(
            ...
            numericInput("Mod1Step3_X1_ran_V","", 1, min = 0, max = 1, step = 0.001)
          )
        }),
    ...
    ######### Run simulation #########
   	...
   	Mod1Step3_output <- reactive({
   	  ...
   	  isolate({  
   	    ...
   	    data <- main(input, "Mod1Step3", session, TRUE) 
    ...
    ######### Display results (graph) #########
     	...
     	output$Mod1Step3_plot <- renderPlot({ 
     	  ...
     	  if(!is.null(data)){
     	    ...
     	    mydata    <- data.frame(dens  = c(data$data_S$Phenotype, 
     	                                      data$data_S$J0,
     	                                      (data$data_S$Be1 * data$data_S$X1) + data$data_S$ME),
     	                            lines = myFactor)
   	...
   	output$Mod1Step3_summary_table <- renderUI({ 
   	  ...
   	  myTable <- data.frame("True"     = c(paste("Total phenotypic variance ($V_",NOT$total,"$) = 1",sep=""),
   	                        ...
   	                        "Estimated" = c(paste("Total Phenotypic variance in sample ($V'_",NOT$total,"$) = ",ifelse(!is.null(Mod1Step3_output()),Mod1Step3_output()$Vp,"...")),
               	                            ...
               	                            paste("Sampled mean of the trait ($\\mu'$) = "        ,ifelse(!is.null(Mod1Step3_output()),Mod1Step3_output()$mean,"...")))
               	)
